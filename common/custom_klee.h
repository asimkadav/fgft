#ifndef MJR_KLEE_CUSTOM_H
#define MJR_KLEE_CUSTOM_H


// MJR ****************************************
// This is a modified version of klee.h
// The original is in ../klee-current/include/klee/klee.h
// We needed to change it to remove some header files that
// it includes.  These headers screw up compilation
// since we are doing lots of fancy preprocessing
// MJR ****************************************
...
extern "C" {
  ...
  /* Add an accesible memory object at a user specified location. It
     ...
     calls. */
  void klee_define_fixed_object(void *addr, unsigned long nbytes);
   ...
   */
  unsigned long klee_choose(unsigned long n);
...
# define klee_assert(expr)                                              \
   ...
   * mode.
   */
  unsigned klee_is_symbolic(unsigned long n);
  ...
  void klee_prioritize(unsigned long condition); // MJR
  void klee_warning(const char *message);
  void klee_warning_once(const char *message);
  void klee_prefer_cex(void *object, unsigned long condition);
...
#define KLEE_GET_VALUE_PROTO(suffix, type)	type klee_get_value##suffix(type expr)
    ...
    KLEE_GET_VALUE_PROTO(ch, char); // MJR Added to KLEE
  KLEE_GET_VALUE_PROTO(f, float);
  KLEE_GET_VALUE_PROTO(d, double);
  KLEE_GET_VALUE_PROTO(l, long);
  KLEE_GET_VALUE_PROTO(ll, long long);
    //KLEE_GET_VALUE_PROTO(_i32, int32_t); // MJR
    //KLEE_GET_VALUE_PROTO(_i64, int64_t); // MJR
...
#undef KLEE_GET_VALUE_PROTO
  ...
  void klee_check_memory_access(const void *address, unsigned long size);
