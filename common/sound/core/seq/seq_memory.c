...
/* static MJR */ inline int snd_seq_pool_available(struct snd_seq_pool *pool)
{
	return pool->total_elements - atomic_read(&pool->counter);
}

/* static MJR */ inline int snd_seq_output_ok(struct snd_seq_pool *pool)
{
...
/* static MJR */ int get_var_len(const struct snd_seq_event *event)
{
...
// EXPORT_SYMBOL(snd_seq_dump_var_event); // MJR
...
/* static MJR */ int seq_copy_in_kernel(char **bufptr, const void *src, int size)
{
...
/* static MJR */ int seq_copy_in_user(char __user **bufptr, const void *src, int size)
{
...
// EXPORT_SYMBOL(snd_seq_expand_var_event); // MJR

/*
 * release this cell, free extended data if available
 */

/* static MJR */ inline void free_cell(struct snd_seq_pool *pool,
			     struct snd_seq_event_cell *cell)
{
	cell->next = pool->free;
	pool->free = cell;
 ...
 */
/* static MJR */ int snd_seq_cell_alloc(struct snd_seq_pool *pool,
...
int snd_seq_pool_done(struct snd_seq_pool *pool)
{
	...
	while (atomic_read(&pool->counter) > 0) {
		if (max_count == 0) {
			snd_mprintk(KERN_WARNING "snd_seq_pool_done timeout: %d cells remain\n", atomic_read(&pool->counter)); // MJR
			break;
		}
...
int /* __init MJR */  snd_sequencer_memory_init(void)
{
	return 0;
}
