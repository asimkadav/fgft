...
/* static MJR */ void port_subs_info_init(struct snd_seq_port_subs_info *grp)
{
						...
						int port)
{
	...
	if (client->num_ports >= SNDRV_SEQ_MAX_PORTS - 1) {
		snd_mprintk(KERN_WARNING "too many ports for client %d\n", client->number); // MJR
		return NULL;
	}
...
/* static MJR */ int subscribe_port(struct snd_seq_client *client,
			  struct snd_seq_client_port *port,
			  struct snd_seq_port_subs_info *grp,
			  struct snd_seq_port_subscribe *info, int send_ack);
/* static MJR */ int unsubscribe_port(struct snd_seq_client *client,
			    struct snd_seq_client_port *port,
			    struct snd_seq_port_subs_info *grp,
			    struct snd_seq_port_subscribe *info, int send_ack);


/* static MJR */ struct snd_seq_client_port *get_client_port(struct snd_seq_addr *addr,
 ...
 */
/* static MJR */ void clear_subscriber_list(struct snd_seq_client *client,
				  struct snd_seq_client_port *port,
				  struct snd_seq_port_subs_info *grp,
				  int grptype)
{
...
/* delete port data */
/* static MJR */ int port_delete(struct snd_seq_client *client,
		       struct snd_seq_client_port *port)
{
			  ...
			  struct snd_seq_port_info * info)
{
	...
	/* set port name */
	if (info->name[0])
		strncpy(port->name, info->name, sizeof(port->name)); // MJR
			  ...
			  struct snd_seq_port_info * info)
{
	...
	/* get port name */
	strncpy(info->name, port->name, sizeof(info->name)); // MJR
...
/* static MJR */ inline int addr_match(struct snd_seq_addr *r, struct snd_seq_addr *s)
{
...
/* static MJR */ int match_subs_info(struct snd_seq_port_subscribe *r,
			      ...
			      int midi_voices, char *portname)
{
	...
	strncpy(portinfo.name, portname ? portname : "Unamed port", // MJR
...
// EXPORT_SYMBOL(snd_seq_event_port_attach); // MJR
...
// EXPORT_SYMBOL(snd_seq_event_port_detach); // MJR
