...
/* static MJR */ void snd_seq_timer_set_tick_resolution(struct snd_seq_timer_tick *tick,
					      int tempo, int ppq)
{
	if (tempo < 1000000)
...
/* static MJR */ void snd_seq_timer_interrupt(struct snd_timer_instance *timeri,
...
int snd_seq_timer_open(struct snd_seq_queue *q)
{
	...
	if (err < 0 && tmr->alsa_id.dev_class != SNDRV_TIMER_CLASS_SLAVE) {
		...
		if (err < 0) {
			snd_mprintk(KERN_ERR "seq fatal error: cannot create timer (%i)\n", err); // MJR
			return err;
		}
	}
...
/* static MJR */ int initialize_timer(struct snd_seq_timer *tmr)
{
	struct snd_timer *t;
	unsigned long freq;
