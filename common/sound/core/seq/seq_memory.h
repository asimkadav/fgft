...
struct snd_seq_event_cell {
    struct snd_seq_event __attribute__((recursive)) event; // MJR
    struct snd_seq_pool *__attribute__((recursive)) pool;				/* used pool */ // MJR
    struct snd_seq_event_cell *__attribute__((recursive)) next;	/* next cell */ // MJR
};
...
struct snd_seq_pool {
    struct snd_seq_event_cell *__attribute__((recursive)) ptr;	/* pointer to first event chunk */ // MJR
    struct snd_seq_event_cell *__attribute__((recursive)) free;	/* pointer to the head of the free list */ // MJR
...
/* static MJR */ inline int snd_seq_unused_cells(struct snd_seq_pool *pool)
{
	return pool ? pool->total_elements - atomic_read(&pool->counter) : 0;
}

/* return total number of allocated cells */
/* static MJR */ inline int snd_seq_total_cells(struct snd_seq_pool *pool)
{
