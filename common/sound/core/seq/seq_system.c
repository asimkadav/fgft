...
static int __attribute__((noderef, address_space(2))) sysclient = -1; // MJR

/* port id numbers for this client */
static int __attribute__((noderef, address_space(1))) announce_port = -1; // MJR



/* fill standard header data, source port & channel are filled in */
/* static MJR */ int setheader(struct snd_seq_event * ev, int client, int port)
{
	if (announce_port < 0)
		return -ENODEV;
...
/* static MJR */ int event_input_timer(struct snd_seq_event * ev, int direct, void *private_data, int atomic, int hop)
{
	return snd_seq_control_queue(ev, atomic, hop);
}

/* register our internal client */
int /* __init MJR */  snd_seq_system_client_init(void)
{
