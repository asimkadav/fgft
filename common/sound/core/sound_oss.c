...
// EXPORT_SYMBOL(snd_lookup_oss_minor_data); // MJR

/* static MJR */ int snd_oss_kernel_minor(int type, struct snd_card *card, int dev)
{
	int minor;
...
// EXPORT_SYMBOL(snd_register_oss_device); // MJR
...
// EXPORT_SYMBOL(snd_unregister_oss_device); // MJR
...
/* static MJR */ const char *snd_oss_device_type_name(int type)
{
	switch (type) {
	case SNDRV_OSS_DEVICE_TYPE_MIXER:
		return "mixer";
...
/* static MJR */ void snd_minor_info_oss_read(struct snd_info_entry *entry,
...
int /* __init MJR */  snd_minor_info_oss_init(void)
{
