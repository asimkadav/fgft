...
//#include <linux/pci.h> // MJR
#include "../../../../common/mpci-bus.h" // MJR
...
/* static MJR */ void update_power_regs(struct snd_ac97 *ac97);
#ifdef CONFIG_SND_AC97_POWER_SAVE
...
/* static MJR */ int snd_ac97_valid_reg(struct snd_ac97 *ac97, unsigned short reg)
{
...
// EXPORT_SYMBOL(snd_ac97_write); // MJR
...
/* static MJR */ inline unsigned short snd_ac97_read_cache(struct snd_ac97 *ac97, unsigned short reg)
{
...
// EXPORT_SYMBOL(snd_ac97_read); // MJR
...
// EXPORT_SYMBOL(snd_ac97_write_cache); // MJR
...
// EXPORT_SYMBOL(snd_ac97_update); // MJR
...
// EXPORT_SYMBOL(snd_ac97_update_bits); // MJR
...
/* static MJR */ int snd_ac97_ad18xx_update_pcm_bits(struct snd_ac97 *ac97, int codec, unsigned short mask, unsigned short value)
{
	int change;
...
/* static MJR */ int snd_ac97_info_enum_double(struct snd_kcontrol *kcontrol,
...
/* static MJR */ int snd_ac97_get_enum_double(struct snd_kcontrol *kcontrol,
...
/* static MJR */ int snd_ac97_put_enum_double(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
	struct ac97_enum *e = (struct ac97_enum *)kcontrol->private_value;
...
/* static MJR */ int snd_ac97_page_save(struct snd_ac97 *ac97, int reg, struct snd_kcontrol *kcontrol)
{
	int page_save = -1;
...
/* static MJR */ void snd_ac97_page_restore(struct snd_ac97 *ac97, int page_save)
{
	if (page_save >= 0) {
...
/* static MJR */ int snd_ac97_info_volsw(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_info *uinfo)
{
...
/* static MJR */ int snd_ac97_get_volsw(struct snd_kcontrol *kcontrol,
			      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
...
/* static MJR */ int snd_ac97_put_volsw(struct snd_kcontrol *kcontrol,
			      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
	int reg = kcontrol->private_value & 0xff;
...
/* static MJR */ void set_inv_eapd(struct snd_ac97 *ac97, struct snd_kcontrol *kctl)
{
...
/* static MJR */ int snd_ac97_spdif_mask_info(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
...
/* static MJR */ int snd_ac97_spdif_cmask_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
/* static MJR */ int snd_ac97_spdif_pmask_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
/* static MJR */ int snd_ac97_spdif_default_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
...
/* static MJR */ int snd_ac97_spdif_default_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
	unsigned int new = 0;
...
/* static MJR */ int snd_ac97_put_spsa(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
	int reg = kcontrol->private_value & 0xff;
...
/* static MJR */ int snd_ac97_ad18xx_pcm_info_bits(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
	struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
...
/* static MJR */ int snd_ac97_ad18xx_pcm_get_bits(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
...
/* static MJR */ int snd_ac97_ad18xx_pcm_put_bits(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
	int codec = kcontrol->private_value & 3;
	int lshift = (kcontrol->private_value >> 8) & 0x0f;
...
/* static MJR */ int snd_ac97_ad18xx_pcm_info_volume(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
...
/* static MJR */ int snd_ac97_ad18xx_pcm_get_volume(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
	int codec = kcontrol->private_value & 3;
...
/* static MJR */ int snd_ac97_ad18xx_pcm_put_volume(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
	int codec = kcontrol->private_value & 3;
...
/* static MJR */ void snd_ac97_powerdown(struct snd_ac97 *ac97);

/* static MJR */ int snd_ac97_bus_free(struct snd_ac97_bus *bus)
{
	if (bus) {
...
/* static MJR */ int snd_ac97_bus_dev_free(struct snd_device *device)
{
	struct snd_ac97_bus *bus = device->device_data;
	return snd_ac97_bus_free(bus);
}

/* static MJR */ int snd_ac97_free(struct snd_ac97 *ac97)
{
	if (ac97) {
#ifdef CONFIG_SND_AC97_POWER_SAVE
...
/* static MJR */ int snd_ac97_dev_free(struct snd_device *device)
{
...
/* static MJR */ int snd_ac97_try_volume_mix(struct snd_ac97 * ac97, int reg)
{
...
/* static MJR */ void check_volume_resolution(struct snd_ac97 *ac97, int reg, unsigned char *lo_max, unsigned char *hi_max)
{
...
/* static MJR */ int snd_ac97_try_bit(struct snd_ac97 * ac97, int reg, int bit)
{
...
/* static MJR */ void snd_ac97_change_volume_params2(struct snd_ac97 * ac97, int reg, int shift, unsigned char *max)
{
...
/* static MJR */ inline int printable(unsigned int x)
{
	x &= 0xff;
	if (x < ' ' || x >= 0x71) {
		if (x <= 0x89)
...
/* static MJR */ struct snd_kcontrol *snd_ac97_cnew(const struct snd_kcontrol_new *_template,
 ...
 */
/* static MJR */ int snd_ac97_cmute_new_stereo(struct snd_card *card, char *name, int reg,
 ...
 * set dB information
 */
/* static MJR */ const DECLARE_TLV_DB_SCALE(db_scale_4bit, -4500, 300, 0);
/* static MJR */ const DECLARE_TLV_DB_SCALE(db_scale_5bit, -4650, 150, 0);
/* static MJR */ const DECLARE_TLV_DB_SCALE(db_scale_6bit, -9450, 150, 0);
/* static MJR */ const DECLARE_TLV_DB_SCALE(db_scale_5bit_12db_max, -3450, 150, 0);
/* static MJR */ const DECLARE_TLV_DB_SCALE(db_scale_rec_gain, 0, 150, 0);

/* static MJR */ const unsigned int *find_db_scale(unsigned int maxval)
{
	switch (maxval) {
...
/* static MJR */ void set_tlv_db_scale(struct snd_kcontrol *kctl, const unsigned int *tlv)
{
	kctl->tlv.p = tlv;
	if (tlv)
 ...
 */
/* static MJR */ int snd_ac97_cvol_new(struct snd_card *card, char *name, int reg, unsigned int lo_max,
 ...
 */
/* static MJR */ int snd_ac97_cmix_new_stereo(struct snd_card *card, const char *pfx,
...
/* static MJR */ unsigned int snd_ac97_determine_spdif_rates(struct snd_ac97 *ac97);

/* static MJR */ int snd_ac97_mixer_build(struct snd_ac97 * ac97)
{
...
/* static MJR */ int snd_ac97_modem_build(struct snd_card *card, struct snd_ac97 * ac97)
{
	int err, idx;
...
/* static MJR */ int snd_ac97_test_rate(struct snd_ac97 *ac97, int reg, int shadow_reg, int rate)
{
	unsigned short val;
	unsigned int tmp;
...
/* static MJR */ void snd_ac97_determine_rates(struct snd_ac97 *ac97, int reg, int shadow_reg, unsigned int *r_result)
{
...
/* static MJR */ const struct ac97_codec_id *look_for_codec_id(const struct ac97_codec_id *table,
						     unsigned int id)
{
...
// EXPORT_SYMBOL(snd_ac97_get_short_name); // MJR

/* wait for a while until registers are accessible after RESET
 * return 0 if ok, negative not ready
 */
/* static MJR */ int ac97_reset_wait(struct snd_ac97 *ac97, int timeout, int with_modem)
{
		 ...
		 void *private_data, struct snd_ac97_bus **rbus)
{
	...
	static struct snd_device_ops __attribute__((noderef, address_space(2))) dev_ops = { // MJR
...
// EXPORT_SYMBOL(snd_ac97_bus); // MJR

/* stop no dev release warning */
/* static MJR */ void ac97_device_release(struct device * dev)
{
}

/* register ac97 codec to bus */
/* static MJR */ int snd_ac97_dev_register(struct snd_device *device)
{
    	struct snd_ac97 *ac97 = device->device_data;
	int err;
        char MJR_temp[128];        // MJR

	ac97->dev.bus = &ac97_bus_type;
	ac97->dev.parent = ac97->bus->card->dev;
	ac97->dev.release = ac97_device_release;
        // MJR ------>
        sprintf (MJR_temp, "%d-%d:%s", ac97->bus->card->number, ac97->num,
                 snd_ac97_get_short_name(ac97));
	dev_set_name(&ac97->dev, MJR_temp);
//	dev_set_name(&ac97->dev, "%d-%d:%s",
//		     ac97->bus->card->number, ac97->num,
//		     snd_ac97_get_short_name(ac97));
        // MJR <--------
	if ((err = device_register(&ac97->dev)) < 0) {
		snd_mprintk(KERN_ERR "Can't register ac97 bus\n"); // MJR
		ac97->dev.bus = NULL;
		return err;
	}
/*
        // MJR_CHECK Test error device_register / put_device
        memset (&ac97->dev, 0, sizeof (ac97->dev));
...
/* static MJR */ int snd_ac97_dev_disconnect(struct snd_device *device)
{
	struct snd_ac97 *ac97 = device->device_data;
	if (ac97->dev.bus)
...
#ifdef CONFIG_SND_AC97_POWER_SAVE
/* static MJR */ void do_update_power(struct work_struct *work)
{
	update_power_regs(
...
int snd_ac97_mixer(struct snd_ac97_bus *bus, struct snd_ac97_template *template, struct snd_ac97 **rac97)
{
	...
	unsigned int reg;
	const struct ac97_codec_id *pid;
	static struct snd_device_ops __attribute__((noderef, address_space(2))) ops = { // MJR
	...
	if (bus->ops->wait)
		...
		if (err < 0) {
			snd_mprintk(KERN_WARNING "AC'97 %d does not respond - RESET\n", ac97->num); // MJR
	...
	if (! (ac97->scaps & AC97_SCAP_DETECT_BY_VENDOR) &&
		...
		snd_mprintk(KERN_ERR "AC'97 %d access is not valid [0x%x], removing mixer.\n", ac97->num, ac97->id); // MJR
		snd_ac97_free(ac97);
		return -EIO;
	}
	pid = look_for_codec_id(snd_ac97_codec_ids, ac97->id);
	...
	if (!ac97_is_audio(ac97) && !ac97_is_modem(ac97)) {
		if (!(ac97->scaps & (AC97_SCAP_SKIP_AUDIO|AC97_SCAP_SKIP_MODEM)))
			snd_mprintk(KERN_ERR "AC'97 %d access error (not audio or modem codec)\n", ac97->num); // MJR
		snd_ac97_free(ac97);
		return -EACCES;
	}
	...
	if (ac97_is_audio(ac97)) {
		...
		snd_mprintk(KERN_WARNING "AC'97 %d analog subsections not ready\n", ac97->num); // MJR
	}
	...
	if (ac97_is_modem(ac97)) {
		...
		snd_mprintk(KERN_WARNING "MC'97 %d converters and GPIO not ready (0x%x)\n", ac97->num, snd_ac97_read(ac97, AC97_EXTENDED_MSTATUS)); // MJR
	}
...
// EXPORT_SYMBOL(snd_ac97_mixer); // MJR
...
// EXPORT_SYMBOL(snd_ac97_update_power); // MJR
...
// EXPORT_SYMBOL(snd_ac97_suspend); // MJR

/*
 * restore ac97 status
 */
/* static MJR */ void snd_ac97_restore_status(struct snd_ac97 *ac97)
{
	int i;
 ...
 */
/* static MJR */ void snd_ac97_restore_iec958(struct snd_ac97 *ac97)
{
	if (ac97->ext_id & AC97_EI_SPDIF) {
...
// EXPORT_SYMBOL(snd_ac97_resume); // MJR
#endif
 ...
 * Hardware tuning
 */
/* static MJR */ void set_ctl_name(char *dst, const char *src, const char *suffix)
{
	if (suffix)
...
/* static MJR */ int snd_ac97_remove_ctl(struct snd_ac97 *ac97, const char *name,
			       const char *suffix)
{
...
/* static MJR */ struct snd_kcontrol *ctl_find(struct snd_ac97 *ac97, const char *name, const char *suffix)
{
...
/* static MJR */ int snd_ac97_rename_ctl(struct snd_ac97 *ac97, const char *src,
...
/* static MJR */ void snd_ac97_rename_vol_ctl(struct snd_ac97 *ac97, const char *src,
				    const char *dst)
{
...
/* swap controls */
/* static MJR */ int snd_ac97_swap_ctl(struct snd_ac97 *ac97, const char *s1,
...
/* static MJR */ int bind_hp_volsw_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
/* static MJR */ int tune_hp_only(struct snd_ac97 *ac97)
{
...
/* static MJR */ int tune_swap_hp(struct snd_ac97 *ac97)
{
	if (ctl_find(ac97, "Headphone Playback Switch", NULL) == NULL)
		return -ENOENT;
...
/* static MJR */ int tune_swap_surround(struct snd_ac97 *ac97)
{
...
/* static MJR */ int tune_ad_sharing(struct snd_ac97 *ac97)
{
	unsigned short scfg;
	if ((ac97->id & 0xffffff00) != 0x41445300) {
		snd_mprintk(KERN_ERR "ac97_quirk AD_SHARING is only for AD codecs\n"); // MJR
		return -EINVAL;
	}
...
/* static MJR */ int tune_alc_jack(struct snd_ac97 *ac97)
{
	if ((ac97->id & 0xffffff00) != 0x414c4700) {
		snd_mprintk(KERN_ERR "ac97_quirk ALC_JACK is only for Realtek codecs\n"); // MJR
		return -EINVAL;
	}
...
/* static MJR */ int tune_inv_eapd(struct snd_ac97 *ac97)
{
...
/* static MJR */ int master_mute_sw_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	int err = snd_ac97_put_volsw(kcontrol, ucontrol);
	if (err > 0) {
		struct snd_ac97 *ac97 = snd_kcontrol_chip(kcontrol);
...
/* static MJR */ int tune_mute_led(struct snd_ac97 *ac97)
{
	struct snd_kcontrol *msw = ctl_find(ac97, "Master Playback Switch", NULL);
	if (! msw)
		return -ENOENT;
...
/* static MJR */ int hp_master_mute_sw_put(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
...
/* static MJR */ int tune_hp_mute_led(struct snd_ac97 *ac97)
{
	struct snd_kcontrol *msw = ctl_find(ac97, "Master Playback Switch", NULL);
	struct snd_kcontrol *mvol = ctl_find(ac97, "Master Playback Volume", NULL);
	if (! msw || ! mvol)
...
/* static MJR */ int apply_quirk(struct snd_ac97 *ac97, int type)
{
	if (type <= 0)
		return 0;
...
/* static MJR */ int apply_quirk_str(struct snd_ac97 *ac97, const char *typestr)
{
	int i;
...
int snd_ac97_tune_hardware(struct snd_ac97 *ac97, struct ac97_quirk *quirk, const char *override)
{
	...
	if (override && strcmp(override, "-1") && strcmp(override, "default")) {
		...
		if (result < 0)
			snd_mprintk(KERN_ERR "applying quirk type %s failed (%d)\n", override, result); // MJR
		return result;
	}
	...
	for (; quirk->subvendor; quirk++) {
		...
		if ((! quirk->mask && quirk->subdevice == ac97->subsystem_device) ||
			...
			if (result < 0)
				snd_mprintk(KERN_ERR "applying quirk type %d for %s failed (%d)\n", quirk->type, quirk->name, result); // MJR
			return result;
		}
	}
	return 0;
...
// EXPORT_SYMBOL(snd_ac97_tune_hardware); // MJR

/*
 *  INIT part
 */

/* static MJR */ int /* __init MJR */  alsa_ac97_init(void)
{
	return 0;
}

/* static MJR */ void __exit alsa_ac97_exit(void)
{
}

//module_init(alsa_ac97_init) // MJR
//module_exit(alsa_ac97_exit) // MJR
