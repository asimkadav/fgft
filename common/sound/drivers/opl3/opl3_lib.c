...
/* static MJR */ void snd_opl2_command(struct snd_opl3 * opl3, unsigned short cmd, unsigned char val)
{
	unsigned long flags;
	unsigned long port;
...
/* static MJR */ void snd_opl3_command(struct snd_opl3 * opl3, unsigned short cmd, unsigned char val)
{
	unsigned long flags;
	unsigned long port;
...
/* static MJR */ int snd_opl3_detect(struct snd_opl3 * opl3)
{
	/*
...
/* static MJR */ int snd_opl3_timer1_start(struct snd_timer * timer)
{
	unsigned long flags;
	unsigned char tmp;
	unsigned int ticks;
...
/* static MJR */ int snd_opl3_timer1_stop(struct snd_timer * timer)
{
	unsigned long flags;
	unsigned char tmp;
	struct snd_opl3 *opl3;
...
/* static MJR */ int snd_opl3_timer2_start(struct snd_timer * timer)
{
	unsigned long flags;
	unsigned char tmp;
	unsigned int ticks;
...
/* static MJR */ int snd_opl3_timer2_stop(struct snd_timer * timer)
{
	unsigned long flags;
	unsigned char tmp;
	struct snd_opl3 *opl3;
...
/* static MJR */ int snd_opl3_timer1_init(struct snd_opl3 * opl3, int timer_no)
{
...
// EXPORT_SYMBOL(snd_opl3_interrupt); // MJR

/*

 */

/* static MJR */ int snd_opl3_free(struct snd_opl3 *opl3)
{
	if (snd_BUG_ON(!opl3))
		return -ENXIO;
...
/* static MJR */ int snd_opl3_dev_free(struct snd_device *device)
{
		 ...
		 struct snd_opl3 **ropl3)
{
	...
	*ropl3 = NULL;
	...
	if (opl3 == NULL) {
		snd_mprintk(KERN_ERR "opl3: cannot allocate\n"); // MJR
		return -ENOMEM;
	}
...
// EXPORT_SYMBOL(snd_opl3_new); // MJR
...
// EXPORT_SYMBOL(snd_opl3_init); // MJR
...
// EXPORT_SYMBOL(snd_opl3_create); // MJR
...
// EXPORT_SYMBOL(snd_opl3_timer_new); // MJR
...
// EXPORT_SYMBOL(snd_opl3_hwdep_new); // MJR

/*
 *  INIT part
 */

/* static MJR */ int __init alsa_opl3_init(void)
{
	return 0;
}

/* static MJR */ void __exit alsa_opl3_exit(void)
{
}

// module_init(alsa_opl3_init) // MJR
// module_exit(alsa_opl3_exit) // MJR
