				...
				u32 value)
{
	...
	if ((reg > 0x7f) || (value > 0x1ff)) {
		snd_mprintk(KERN_ERR "i2c_write: invalid values.\n");
		return -EINVAL;
	}

	tmp = reg << 25 | value << 16;
	// snd_mprintk("I2C-write:reg=0x%x, value=0x%x\n", reg, value);
	...
	if (retry == 10) {
		snd_mprintk(KERN_ERR "Writing to ADC failed!\n");
		return -EINVAL;
	}
						...
						int channel_id)
{
	...
	channel->use = 1;
        //mprintk("open:channel_id=%d, chip=%p, channel=%p\n",channel_id, chip, channel);
					       ...
					       int channel_id)
{
	...
	epcm = kzalloc(sizeof(*epcm), GFP_KERNEL);
	if (epcm == NULL) {
		snd_mprintk(KERN_ERR "open_capture_channel: failed epcm alloc\n");
		return -ENOMEM;
        }
	epcm->emu = chip;
	epcm->substream = substream;
...
static int snd_ca0106_pcm_prepare_playback(struct snd_pcm_substream *substream)
{
	...
	int i;
        ...
        //snd_mprintk("prepare:channel_number=%d, rate=%d, format=0x%x, channels=%d, buffer_size=%ld, period_size=%ld, periods=%u, frames_to_bytes=%d\n",channel, runtime->rate, runtime->format, runtime->channels, runtime->buffer_size, runtime->period_size, runtime->periods, frames_to_bytes(runtime, 1));
        //snd_mprintk("dma_addr=%x, dma_area=%p, table_base=%p\n",runtime->dma_addr, runtime->dma_area, table_base);
	//snd_mprintk("dma_addr=%x, dma_area=%p, dma_bytes(size)=%x\n",emu->buffer.addr, emu->buffer.area, emu->buffer.bytes);
...
static int snd_ca0106_pcm_prepare_capture(struct snd_pcm_substream *substream)
{
	...
	snd_ca0106_ptr_write(emu, 0x71, 0, reg71);
        ...
        //mprintk("prepare:channel_number=%d, rate=%d, format=0x%x, channels=%d, buffer_size=%ld, period_size=%ld, frames_to_bytes=%d\n",channel, runtime->rate, runtime->format, runtime->channels, runtime->buffer_size, runtime->period_size,  frames_to_bytes(runtime, 1));
				    ...
				    int cmd)
{
	...
	default:
		...
		/* snd_mprintk("channel=%d\n",channel); */
		epcm->running = running;
		basic |= (0x1 << channel);
		extended |= (0x10 << channel);
                snd_pcm_trigger_done(s, substream);
        }
	/* snd_mprintk("basic=0x%x, extended=0x%x\n",basic, extended); */
...
snd_ca0106_pcm_pointer_playback(struct snd_pcm_substream *substream)
{
	...
	//mprintk("ptr1 = 0x%lx, ptr2=0x%lx, ptr=0x%lx, buffer_size = 0x%x, period_size = 0x%x, bits=%d, rate=%d\n", ptr1, ptr2, ptr, (int)runtime->buffer_size, (int)runtime->period_size, (int)runtime->frame_bits, (int)runtime->rate);
...
static struct snd_ac97_bus_ops __attribute__((noderef, address_space(2))) ops = {
...
static irqreturn_t snd_ca0106_interrupt(int irq, void *dev_id)
{
	...
	//snd_mprintk("interrupt status = 0x%08x, stat76=0x%08x\n", status, stat76);
	//snd_mprintk("ptr=0x%08x\n",snd_ca0106_ptr_read(chip, PLAYBACK_POINTER, 0));
...
/* FIXME: Select the correct substream for period elapsed */
			if(pchannel->use) {
				...
				//mprintk(KERN_INFO "interrupt [%d] used\n", i);
                        }
		}
	        //mprintk(KERN_INFO "channel=%p\n",pchannel);
	        //mprintk(KERN_INFO "interrupt stat76[%d] = %08x, use=%d, channel=%d\n", i, stat76, pchannel->use, pchannel->number);
		mask <<= 1;
	}
...
static void ca0106_init_chip(struct snd_ca0106 *chip, int resume)
{
	...
	if (chip->details->i2c_adc == 1) {
		...
		/* snd_mprintk("I2C:array size=0x%x\n", size); */
		for (n = 0; n < size; n++)
					 ...
					 struct snd_ca0106 **rchip)
{
	...
	*rchip = NULL;
	...
	if (pci_set_dma_mask(pci, DMA_32BIT_MASK) < 0 ||
		...
		mprintk(KERN_ERR "error to set 32bit mask DMA\n");
	...
	if (!chip->res_port) {
		...
		mprintk(KERN_ERR "cannot allocate the port\n");
		return -EBUSY;
	}
	...
	if (request_irq(pci->irq, snd_ca0106_interrupt,
		...
		snd_ca0106_free(chip);
		mprintk(KERN_ERR "cannot grab irq\n");
		return -EBUSY;
	}
	chip->irq = pci->irq;
	...
	mprintk(KERN_INFO "snd-ca0106: Model %04x Rev %08x Serial %08x\n",
	...
	chip->details = c;
	if (subsystem[dev]) {
		mprintk(KERN_INFO "snd-ca0106: Sound card name=%s, "
...
static int snd_ca0106_checkpoint(struct pci_dev *pci)  
{
...
static int snd_ca0106_restore(struct pci_dev *pci)
{ 
  struct snd_card *card = pci_get_drvdata(pci);
  struct snd_ca0106 *chip = card->private_data;
  int i;
