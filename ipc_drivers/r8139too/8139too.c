...
struct rtl8139_private {
	//void __iomem		* __attribute__((noderef, address_space(2))) mmio_addr;
	...
	unsigned char		* __attribute__((noderef, address_space(2))) tx_buf[NUM_TX_DESC];	/* Tx bounce buffers */
	unsigned char		* __attribute__((noderef, address_space(2))) tx_bufs;	/* Tx bounce buffer region. */
...
static struct pci_dev* odftpdev = NULL;
				       ...
				       const struct pci_device_id *ent)
{
	...
	tp = netdev_priv(dev);
	tp->dev = dev;
	dev->priv = tp; //ASIMRECOVERY
	odftpdev = pdev; //ASIMRECOVERY
	ioaddr = tp->mmio_addr;
	assert (ioaddr != NULL);
	...
	tp->lock = (spinlock_t *) kmalloc(sizeof(spinlock_t), GFP_ATOMIC); //ASIM 
	tp->rx_lock = (spinlock_t *) kmalloc(sizeof(spinlock_t), GFP_ATOMIC);  //ASIM
	spin_lock_init (tp->lock); //ASIM
	spin_lock_init (tp->rx_lock); //ASIM
...
static int rtl8139_start_xmit (struct sk_buff * __attribute__((noderef, address_space(2))) skb, struct net_device *dev)// __attribute((isolate)) 
{
	//struct rtl8139_private *tp = netdev_priv(dev); ASIM
...
{
	//struct rtl8139_private *np = netdev_priv(dev); ASIM
...
{
	//struct rtl8139_private *np = netdev_priv(dev);
	struct rtl8139_private *np = dev->priv; //ASIM
...
static void rtl8139_set_msglevel(struct net_device *dev, u32 datum) //__attribute((isolate)) 
{
	//struct rtl8139_private *np = netdev_priv(dev); 
	struct rtl8139_private *np = dev->priv; //ASIM
	np->msg_enable = datum;
...
static int rtl8139_get_regs_len(struct net_device *dev) __attribute((isolate))
{
	...
	if (use_io)
		return 0;
	//np = netdev_priv(dev); //ASIM
	np = dev->priv;
	return np->regs_len;
}

static void rtl8139_get_regs(struct net_device *dev, struct ethtool_regs *regs, void * __attribute__((noderef, address_space(2)))regbuf) __attribute((isolate))  //ASIM
{
	struct rtl8139_private *np;
...
int r8139too_restore()	
{
	...
	if (!netif_running (dev))
		return 0;
	//pci_set_power_state (odftpdev, PCI_D0);
	rtl8139_init_ring (dev);
	rtl8139_hw_start (dev);
