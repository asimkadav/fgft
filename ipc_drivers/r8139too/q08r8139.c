				       ...
				       const struct pci_device_id *ent)
{
	...
	tp->lock = (spinlock_t *) kmalloc(sizeof(spinlock_t), GFP_ATOMIC); 
	tp->rx_lock = (spinlock_t *) kmalloc(sizeof(spinlock_t), GFP_ATOMIC); 
...
static u32 rtl8139_get_link(struct net_device *dev) __attribute((isolate))
{
	//struct rtl8139_private *np = netdev_priv(dev);
	struct rtl8139_private *np = dev->priv; //ASIM
...
static u32 rtl8139_get_msglevel(struct net_device *dev) __attribute((isolate))  //ASIM
{
	//struct rtl8139_private *np = netdev_priv(dev);
	struct rtl8139_private *np = dev->priv; //ASIM
...
static void rtl8139_set_msglevel(struct net_device *dev, u32 datum) __attribute((isolate))//ASIM
{
	//struct rtl8139_private *np = netdev_priv(dev); 
	struct rtl8139_private *np = dev->priv; //ASIM
	np->msg_enable = datum;
