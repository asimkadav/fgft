...
static char *(__attribute__((noderef, address_space(2))) id)[SNDRV_CARDS] = SNDRV_DEFAULT_STR;	/* ID for this card */
...
struct ensoniq {
    ...
    struct { // MJR Was union
#ifdef CHIP1371
		struct {
                    struct snd_ac97 *__attribute__((recursive)) ac97; // MJR
		} __attribute__((recursive)) es1371; // MJR
#else
		struct {
			int pclkdiv_lock;
			struct snd_ak4531 *ak4531;
		} __attribute__((recursive)) es1370; // MJR
#endif
    } __attribute__((recursive)) u; // MJR

	struct pci_dev *pci;
    struct snd_card *__attribute__((recursive)) card; // MJR
    struct snd_pcm *__attribute__((recursive)) pcm1;	/* DAC1/ADC PCM */ // MJR
    struct snd_pcm *__attribute__((recursive)) pcm2;	/* DAC2 PCM */ // MJR
    struct snd_pcm_substream *__attribute__((recursive)) playback1_substream; // MJR
    struct snd_pcm_substream *__attribute__((recursive)) playback2_substream; // MJR
    struct snd_pcm_substream *__attribute__((recursive)) capture_substream; // MJR
    ...
    struct snd_rawmidi *__attribute__((recursive)) rmidi; // MJR
...
static struct pci_device_id (__attribute__((noderef, address_space(2))) snd_audiopci_ids)[] = { // MJR
#ifdef CHIP1370
...
#define POLL_COUNT	0x10 /* MJR reduced from 0xa000 */

#ifdef CHIP1370
static unsigned int snd_es1370_fixed_rates[] =
	{5512, 11025, 22050, 44100};
static struct snd_pcm_hw_constraint_list __attribute__((noderef, address_space(2))) snd_es1370_hw_constraints_rates = { // MJR
	.count = 4, 
	.list = snd_es1370_fixed_rates,
	.mask = 0,
};
static struct snd_ratnum __attribute__((noderef, address_space(2))) es1370_clock = { // MJR
	.num = ES_1370_SRCLOCK,
	.den_min = 29, 
	.den_max = 353,
	.den_step = 1,
};
static struct snd_pcm_hw_constraint_ratnums __attribute__((noderef, address_space(2))) snd_es1370_hw_constraints_clock = { // MJR
	.nrats = 1,
	.rats = &es1370_clock,
};
#else
static struct snd_ratden __attribute__((noderef, address_space(2))) es1371_dac_clock = { // MJR
	.num_min = 3000 * (1 << 15),
	.num_max = 48000 * (1 << 15),
	.num_step = 3000,
	.den = 1 << 15,
};
static struct snd_pcm_hw_constraint_ratdens __attribute__((noderef, address_space(2))) snd_es1371_hw_constraints_dac_clock = { // MJR
	.nrats = 1,
	.rats = &es1371_dac_clock,
};
static struct snd_ratnum __attribute__((noderef, address_space(2))) es1371_adc_clock = {  // MJR
	.num = 48000 << 15,
	.den_min = 32768, 
	.den_max = 393216,
	.den_step = 1,
};
static struct snd_pcm_hw_constraint_ratnums __attribute__((noderef, address_space(2))) snd_es1371_hw_constraints_adc_clock = { // MJR
	.nrats = 1,
...
static unsigned int snd_es1371_wait_src_ready(struct ensoniq * ensoniq)
{
	...
	printk(KERN_ERR "wait src ready timeout 0x%lx [0x%x]\n",
				   ...
				   unsigned short reg, unsigned short val)
{
	...
	printk(KERN_ERR "codec write timeout, status = 0x%x\n",
				   ...
				   unsigned short reg, unsigned short val)
{
	...
	mutex_unlock(&ensoniq->src_mutex);
	printk(KERN_ERR "codec write timeout at 0x%lx [0x%x]\n",
					    ...
					    unsigned short reg)
{
	...
	for (t = 0; t < POLL_COUNT; t++) {
		if (!(inl(ES_REG(ensoniq, 1371_CODEC)) & ES_1371_CODEC_WIP)) {
			...
			mutex_unlock(&ensoniq->src_mutex);
			if (++fail > 10) {
				printk(KERN_ERR "codec read timeout (final) "
	...
	mutex_unlock(&ensoniq->src_mutex);
	printk(KERN_ERR "es1371: codec read timeout at 0x%lx [0x%x]\n",
		   ES_REG(ensoniq, 1371_CODEC), inl(ES_REG(ensoniq, 1371_CODEC)));
	return 0;
}
...
static struct snd_pcm_hardware __attribute__((noderef, address_space(2))) snd_ensoniq_playback1 =  // MJR
{
...
static struct snd_pcm_hardware __attribute__((noderef, address_space(2))) snd_ensoniq_playback2 = // MJR
{
	.info =			(SNDRV_PCM_INFO_MMAP | SNDRV_PCM_INFO_INTERLEAVED |
				 SNDRV_PCM_INFO_BLOCK_TRANSFER |
...
static struct snd_pcm_hardware __attribute__((noderef, address_space(2))) snd_ensoniq_capture = // MJR
{
	.info =			(SNDRV_PCM_INFO_MMAP | SNDRV_PCM_INFO_INTERLEAVED |
				 SNDRV_PCM_INFO_BLOCK_TRANSFER |
...
static struct snd_pcm_ops __attribute__((noderef, address_space(2))) snd_ensoniq_playback1_ops = { // MJR
...
static struct snd_pcm_ops __attribute__((noderef, address_space(2))) snd_ensoniq_playback2_ops = { // MJR
...
static struct snd_pcm_ops __attribute__((noderef, address_space(2))) snd_ensoniq_capture_ops = { // MJR
...
/* spdif controls */
static struct snd_kcontrol_new (__attribute__((noderef, address_space(2))) snd_es1371_mixer_spdif)[] __devinitdata = { // MJR
...
static struct es1371_quirk (__attribute__((noderef, address_space(2))) es1371_spdif_present)[] __devinitdata = { // MJR
...
static struct snd_pci_quirk (__attribute__((noderef, address_space(2))) ens1373_line_quirk)[] __devinitdata = { // MJR
	SND_PCI_QUIRK_ID(0x1274, 0x2000), /* GA-7DXR */
	SND_PCI_QUIRK_ID(0x1458, 0xa000), /* GA-8IEXP */
	{ 0, 0, 0 } /* end */ // MJR Added 0, 0, 0 due to CIL bug
};
...
static void snd_ensoniq_proc_read(struct snd_info_entry *entry, 
				  struct snd_info_buffer *buffer) __attribute((isolate))
{
...
#ifdef CHIP1371
static struct snd_pci_quirk (__attribute__((noderef, address_space(2))) es1371_amplifier_hack)[] __devinitdata = { // MJR
	SND_PCI_QUIRK_ID(0x107b, 0x2150),	/* Gateway Solo 2150 */
	SND_PCI_QUIRK_ID(0x13bd, 0x100c),	/* EV1938 on Mebius PC-MJ100V */
	SND_PCI_QUIRK_ID(0x1102, 0x5938),	/* Targa Xtender300 */
	SND_PCI_QUIRK_ID(0x1102, 0x8938),	/* IPC Topnote G notebook */
	{ 0, 0, 0 } /* end */ // MJR Added 0, 0, 0 b/c of CIL bug
};

static struct es1371_quirk (__attribute__((noderef, address_space(2))) es1371_ac97_reset_hack)[] = { // MJR
	{ .vid = PCI_VENDOR_ID_ENSONIQ, .did = PCI_DEVICE_ID_ENSONIQ_CT5880, .rev = CT5880REV_CT5880_C },
	{ .vid = PCI_VENDOR_ID_ENSONIQ, .did = PCI_DEVICE_ID_ENSONIQ_CT5880, .rev = CT5880REV_CT5880_D },
	{ .vid = PCI_VENDOR_ID_ENSONIQ, .did = PCI_DEVICE_ID_ENSONIQ_CT5880, .rev = CT5880REV_CT5880_E },
	{ .vid = PCI_VENDOR_ID_ENSONIQ, .did = PCI_DEVICE_ID_ENSONIQ_ES1371, .rev = ES1371REV_CT5880_A },
				     ...
				     struct ensoniq ** rensoniq)
{
	...
	*rensoniq = NULL;
	...
	if (request_irq(pci->irq, snd_audiopci_interrupt, IRQF_SHARED,
		...
		printk(KERN_ERR "unable to grab IRQ %d\n", pci->irq);
	...
	if (snd_dma_alloc_pages(SNDRV_DMA_TYPE_DEV, snd_dma_pci_data(pci),
		...
		printk(KERN_ERR "unable to allocate space for phantom area - dma_bug\n");
		snd_ensoniq_free(ensoniq);
		return -EBUSY;
	}
#endif
...
{
    snd_card_free(pci_get_drvdata(pci)); // MJR
    pci_set_drvdata(pci, NULL); // MJR
}
