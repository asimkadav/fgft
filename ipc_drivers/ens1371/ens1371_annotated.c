...
static struct pci_dev * ens1371_dev = NULL; //ASIMRECOVERY 
...
struct ensoniq {
...
#ifdef CHIP1371
		struct {
			struct snd_ac97 *__attribute__((recursive))ac97; //ASIM
		} __attribute__((recursive)) es1371; //ASIM
#else
		struct {
			int pclkdiv_lock;
			struct snd_ak4531 *ak4531;
		} __attribute__((recursive)) es1370; //ASIM
#endif
	} u;

	struct pci_dev *pci;
	struct snd_card *__attribute__((recursive))  card; //ASIM
	struct snd_pcm *__attribute__((recursive))  pcm1; //ASIM	/* DAC1/ADC PCM */
	struct snd_pcm *__attribute__((recursive))  pcm2; //ASIM	/* DAC2 PCM */
	struct snd_pcm_substream *__attribute__((recursive))  playback1_substream; //ASIM
	struct snd_pcm_substream *__attribute__((recursive))  playback2_substream; //ASIM
	struct snd_pcm_substream *__attribute__((recursive))  capture_substream; //ASIM
	...
	struct snd_rawmidi *__attribute__((recursive))  rmidi; //ASIM
...
{
	//struct ensoniq *ensoniq = snd_pcm_substream_chip(substream);
	struct ensoniq *ensoniq = substream->private_data; //ASIM
...
{
	//struct ensoniq *ensoniq = snd_pcm_substream_chip(substream); //ASIM
...
{
	//struct ensoniq *ensoniq = snd_kcontrol_chip(kcontrol); ASIM
				     ...
				     struct ensoniq ** rensoniq)
{
	...
	*rensoniq = NULL;
	...
	ensoniq->reg_lock = (struct spinlock_t *) kmalloc (sizeof (spinlock_t), GFP_ATOMIC); //ASIM
 	ensoniq->src_mutex = (struct mutex *) kmalloc (sizeof (struct mutex), GFP_ATOMIC);  //ASIM	
	spin_lock_init(ensoniq->reg_lock); // ASIM
	mutex_init(ensoniq->src_mutex); //ASIM
	ensoniq->card = card;
	ensoniq->pci = pci;
	ens1371_dev = pci;
	ensoniq->irq = -1;
