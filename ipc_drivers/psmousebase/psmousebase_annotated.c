...
void psmouse_set_resolution(struct psmouse *psmouse, unsigned int resolution) __attribute((isolate))
{
...
static void psmouse_set_rate(struct psmouse *psmouse, unsigned int rate) __attribute((isolate)) 
{
...
extern odft_is_self_locked(struct mutex *);

static void psmouse_cleanup(struct serio *serio)
{
	struct psmouse *psmouse = serio_get_drvdata(serio);
	struct psmouse *parent = NULL;
	int recovery_context = 0; //ASIMRECOVERY

	if (odft_is_self_locked(&psmouse_mutex) != 0) 		//ASIMRECOVERY	
		mutex_lock(&psmouse_mutex);
	else recovery_context = 1; //ASIMRECOVERY
 ...
 */
	...
	if (parent) {
       ...
       if (recovery_context == 0) {//ASIMRECOVERY	
		mutex_unlock(&psmouse_mutex);
	}
}
...
int psmousebase_restore(void)
{
	...
	psmouse_cleanup(odft_psmouse_serio);
	return 0;
}
...
static ssize_t psmouse_show_int_attr(struct psmouse *psmouse, 
				void *__attribute__((noderef, address_space(2))) offset, char *__attribute__((blob(4))) buf) __attribute((isolate)) //ASIM
{
...
static ssize_t psmouse_set_int_attr(struct psmouse *psmouse, void *__attribute__((noderef, address_space(2)))offset, const char * __attribute__((blob(4)))buf, size_t count) __attribute((isolate))
							//ASIM
{
	unsigned int *field = (unsigned int *)((char *)psmouse + (size_t)offset);
	unsigned long value;
...
static ssize_t psmouse_attr_show_protocol(struct psmouse *psmouse, void *data, char * buf) __attribute((isolate))
{
...
static int psmouse_set_maxproto(const char *val, struct kernel_param *kp) __attribute((isolate))
{
...
static int psmouse_get_maxproto(char *buffer, struct kernel_param *kp) __attribute((isolate))
{
