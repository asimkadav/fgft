...
/*
void * MODIFANNOT(dma_unmap_single) 
    (struct device *dev,
      dma_addr_t addr,
       size_t size,
       int direction)   {
    //MODIFIES(dev->dma_mem);
    MODIFIES(dev->archdata.dma_ops);
    return 0;
}

struct dma_mapping_ops * MODIFANNOT(get_dma_ops)
        (struct device *dev )
{
   MODIFIES(dev->archdata.dma_ops);
}
...
int MODIFANNOT (del_timer_sync)
    (struct timer_list *timer)
{

    MODIFIES(timer->expires);
    MODIFIES(timer->function);
    MODIFIES(timer->data);
    MODIFIES(timer->base);
...
int MODIFANNOT(cancel_work_sync)(struct work_struct *work) {
    MODIFIES(work->func);
//    MODIFIES(work->entry.next);
//    MODIFIES(work->entry.prev);
    return 0;
}

int MODIFANNOT(schedule_work)(struct work_struct *work) {
    MODIFIES(work->func);
//    MODIFIES(work->entry.next);
//    MODIFIES(work->entry.prev);
    return 0;
...
unsigned char * MODIFANNOT(skb_end_pointer) (const struct sk_buff *skb)
{
	MODIFIES(skb->head);
	MODIFIES(skb->end);
	return NULL;
