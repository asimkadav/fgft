...
struct fe_priv {
	...
	void __iomem * __attribute__((noderef, address_space(2))) base;
...
static int reg_delay(struct net_device *dev, int offset, u32 mask, u32 target,
				int delay, int delaymax, const char * __attribute__((nullterm))msg) //ASIM
{
...
static void nv_get_ringparam(struct net_device *dev, struct ethtool_ringparam* ring) __attribute((isolate))
{
	struct fe_priv *np = netdev_priv(dev);
