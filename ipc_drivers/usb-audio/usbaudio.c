...
#define WRAPPERS_SYM_DRIVER_STEP1 // MJR
#include "../../common/wrappers_usb.h" // MJR Wrappers--need them preprocessed
#endif
...
#include <sound/control.h> // MJR For modifs
#include <sound/rawmidi.h> // MJR for modifs
#include <linux/proc_fs.h> // MJR for modif_annots.h

#ifndef REAL_KERNEL_MODULE
#include "modif_annots.h" // MJR
int MJR_middle; // MJR
#endif
...
struct snd_urb_ctx {
    struct urb *__attribute__((recursive)) urb; // MJR
...
struct snd_usb_substream {
    struct snd_usb_stream *__attribute__((recursive)) stream; // MJR
	struct usb_device *dev;
    struct snd_pcm_substream *__attribute__((recursive)) pcm_substream; // MJR
    ...
    char * __attribute__((nullterm)) syncbuf;				/* sync buffer for all sync URBs */ // MJR
...
struct snd_usb_stream {
    ...
    struct snd_pcm *__attribute__((recursive)) pcm; // MJR
	int pcm_index;
	unsigned int fmt_type;		/* USB audio format type (1-3) */
...
// MJR --------------------->
// MJR Moved from usbmidi.c
struct usb_protocol_ops { // Copied from usbmidi.c
...
void snd_usbmidi_us122l_input(struct snd_usb_midi_in_endpoint *ep,
				     uint8_t *buffer, int buffer_length);
void snd_usbmidi_us122l_output(struct snd_usb_midi_out_endpoint *ep);

struct usb_protocol_ops snd_usbmidi_122l_ops = {
	.input = snd_usbmidi_us122l_input,
	.output = snd_usbmidi_us122l_output,
};

void snd_usbmidi_standard_input(struct snd_usb_midi_in_endpoint* ep,
                                uint8_t* buffer, int buffer_length);
void snd_usbmidi_standard_output(struct snd_usb_midi_out_endpoint* ep);
void snd_usbmidi_output_standard_packet(struct urb* urb, uint8_t p0,
                                        uint8_t p1, uint8_t p2, uint8_t p3);

struct usb_protocol_ops snd_usbmidi_standard_ops = {
	.input = snd_usbmidi_standard_input,
	.output = snd_usbmidi_standard_output,
	.output_packet = snd_usbmidi_output_standard_packet,
};

void snd_usbmidi_midiman_input(struct snd_usb_midi_in_endpoint* ep,
                               uint8_t* buffer, int buffer_length);
void snd_usbmidi_output_midiman_packet(struct urb* urb, uint8_t p0,
                                       uint8_t p1, uint8_t p2, uint8_t p3);

struct usb_protocol_ops snd_usbmidi_midiman_ops = {
	.input = snd_usbmidi_midiman_input,
	.output = snd_usbmidi_standard_output, 
	.output_packet = snd_usbmidi_output_midiman_packet,
};

void snd_usbmidi_maudio_broken_running_status_input(
...
void snd_usbmidi_cme_input(struct snd_usb_midi_in_endpoint *ep,
                           uint8_t *buffer, int buffer_length);

struct usb_protocol_ops snd_usbmidi_cme_ops = {
	.input = snd_usbmidi_cme_input,
	.output = snd_usbmidi_standard_output,
	.output_packet = snd_usbmidi_output_standard_packet,
};

void snd_usbmidi_emagic_input(struct snd_usb_midi_in_endpoint* ep,
                              uint8_t* buffer, int buffer_length);
...
struct usb_protocol_ops snd_usbmidi_emagic_ops = {
...
void snd_usbmidi_novation_input(struct snd_usb_midi_in_endpoint* ep,
                                uint8_t* buffer, int buffer_length);
...
void snd_usbmidi_raw_input(struct snd_usb_midi_in_endpoint* ep,
                           uint8_t* buffer, int buffer_length);
void snd_usbmidi_raw_output(struct snd_usb_midi_out_endpoint* ep);
struct usb_protocol_ops snd_usbmidi_raw_ops = {
	.input = snd_usbmidi_raw_input,
	.output = snd_usbmidi_raw_output,
};

// MJR Moved out from snd_usb_create_quirk:
...
static struct snd_usb_audio *__attribute__((noderef, address_space(2))) usb_chip[SNDRV_CARDS]; // MJR
...
/*
 */
static struct snd_urb_ops (__attribute__((noderef, address_space(2))) audio_urb_ops)[2] = { // MJR_new
	{
...
static struct snd_urb_ops (__attribute__((noderef, address_space(2))) audio_urb_ops_high_speed)[2] = { // MJR_new
	{
		.prepare =	prepare_nodata_playback_urb,
		.retire =	retire_playback_urb,
					     ...
					     unsigned long offset)
{
	...
	//return vmalloc_to_page(pageptr); // MJR
        return virt_to_page(pageptr); // MJR
}
...
static int snd_pcm_alloc_vmalloc_buffer(struct snd_pcm_substream *subs, size_t size)
{
	struct snd_pcm_runtime *runtime = subs->runtime;
        int new_size = (1 << get_order(size)) * 4096; // MJR added this line

        uprintk ("MJR Added this: Size: %d, new-size: %d\n", size, new_size);

	if (runtime->dma_area) {
            if (runtime->dma_bytes >= new_size) // MJR was size
			return 0; /* already large enough */
		//vfree(runtime->dma_area);
                free_pages(runtime->dma_area, -1);
	}
	//runtime->dma_area = vmalloc(size); // MJR
        runtime->dma_area = __get_free_pages(GFP_KERNEL, get_order(size)); // MJR
	if (!runtime->dma_area)
		return -ENOMEM;
	runtime->dma_bytes = new_size; // MJR was size
	return 0;
}
...
static int snd_pcm_free_vmalloc_buffer(struct snd_pcm_substream *subs)
{
	...
	//vfree(runtime->dma_area); // MJR
        free_pages(runtime->dma_area, -1); // MJR
...
static int start_urbs(struct snd_usb_substream *subs, struct snd_pcm_runtime *runtime)
{
	...
	for (i = 0; i < subs->nurbs; i++) {
		...
		if (subs->ops.prepare(subs, runtime, subs->dataurb[i].urb) < 0) {
			mprintk(KERN_ERR "cannot prepare datapipe for urb %d\n", i);
			goto __error;
		}
	}
	if (subs->syncpipe) {
		for (i = 0; i < SYNC_URBS; i++) {
			...
			if (subs->ops.prepare_sync(subs, runtime, subs->syncurb[i].urb) < 0) {
				mprintk(KERN_ERR "cannot prepare syncpipe for urb %d\n", i);
				goto __error;
			}
		}
	}
	...
	for (i = 0; i < subs->nurbs; i++) {
		...
		if (err < 0) {
			mprintk(KERN_ERR "cannot submit datapipe "
				   "for urb %d, error %d: %s\n",
	...
	if (subs->syncpipe) {
		for (i = 0; i < SYNC_URBS; i++) {
			...
			if (err < 0) {
				mprintk(KERN_ERR "cannot submit syncpipe "
					   "for urb %d, error %d: %s\n",
					   i, err, usb_error_string(err));
				goto __error;
			}
...
static int wait_clear_urbs(struct snd_usb_substream *subs)
{
	...
	if (alive)
		mprintk(KERN_ERR "timeout: still %d active urbs..\n", alive);
	return 0;
}
			  ...
			  struct audioformat *fmt)
{
	...
	if (fmt->attributes & EP_CS_ATTR_PITCH_CONTROL) {
		...
		if ((err = snd_usb_ctl_msg(dev, usb_sndctrlpipe(dev, 0), SET_CUR,
			...
			mprintk(KERN_ERR "%d:%d:%d: cannot set enable PITCH\n",
				   dev->devnum, iface, ep);
			return err;
		}
	}
				...
				struct audioformat *fmt, int rate)
{
	...
	if (fmt->attributes & EP_CS_ATTR_SAMPLE_RATE) {
		...
		if ((err = snd_usb_ctl_msg(dev, usb_sndctrlpipe(dev, 0), SET_CUR,
			...
			mprintk(KERN_ERR "%d:%d:%d: cannot set freq %d to ep 0x%x\n",
		...
		if ((err = snd_usb_ctl_msg(dev, usb_rcvctrlpipe(dev, 0), GET_CUR,
					   ...
					   SAMPLING_FREQ_CONTROL << 8, ep, data, 3, 1000)) < 0) {
			mprintk(KERN_WARNING "%d:%d:%d: cannot get freq at ep 0x%x\n",
...
static int set_format(struct snd_usb_substream *subs, struct audioformat *fmt)
{
	...
	if (subs->interface >= 0 && subs->interface != fmt->iface) {
		if (usb_set_interface(subs->dev, subs->interface, 0) < 0) {
			mprintk(KERN_ERR "%d:%d:%d: return to setting 0 failed\n",
	...
	if (subs->interface != fmt->iface || subs->format != fmt->altset_idx) {
		if (usb_set_interface(dev, fmt->iface, fmt->altsetting) < 0) {
			mprintk(KERN_ERR "%d:%d:%d: usb_set_interface failed\n",
				   dev->devnum, fmt->iface, fmt->altsetting);
			return -EIO;
		}
	...
	if (((is_playback && attr == EP_ATTR_ASYNC) ||
		...
		if ((get_endpoint(alts, 1)->bmAttributes & USB_ENDPOINT_XFERTYPE_MASK) != 0x01 ||
			...
			mprintk(KERN_ERR "%d:%d:%d : invalid synch pipe\n",
				   dev->devnum, fmt->iface, fmt->altsetting);
			return -EINVAL;
		}
...
static int snd_usb_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	if (! subs->cur_audiofmt) {
		mprintk(KERN_ERR "usbaudio: no format is specified!\n");
		return -ENXIO;
	}
...
static struct snd_pcm_hardware __attribute__((noderef, address_space(2))) snd_usb_hardware = // MJR_new
{
...
#define hwc_debug(fmt, args...) mprintk(KERN_DEBUG fmt, ##args)
#else
...
static int setup_hw_info(struct snd_pcm_runtime *runtime, struct snd_usb_substream *subs)
{
	...
	if (err < 0)
		...
		if ((err = snd_pcm_hw_rule_add_MJR2(runtime, 0, SNDRV_PCM_HW_PARAM_RATE,
					       hw_rule_rate, subs,
					       SNDRV_PCM_HW_PARAM_FORMAT,
					       SNDRV_PCM_HW_PARAM_CHANNELS,
					       -1)) < 0)
			return err;
		if ((err = snd_pcm_hw_rule_add_MJR2(runtime, 0, SNDRV_PCM_HW_PARAM_CHANNELS,
					       hw_rule_channels, subs,
					       SNDRV_PCM_HW_PARAM_FORMAT,
					       SNDRV_PCM_HW_PARAM_RATE,
					       -1)) < 0)
			return err;
		if ((err = snd_pcm_hw_rule_add_MJR2(runtime, 0, SNDRV_PCM_HW_PARAM_FORMAT,
					       hw_rule_format, subs,
					       SNDRV_PCM_HW_PARAM_RATE,
					       SNDRV_PCM_HW_PARAM_CHANNELS,
					       -1)) < 0)
...
static struct snd_pcm_ops __attribute__((noderef, address_space(2))) snd_usb_playback_ops = { // MJR_new
...
static struct snd_pcm_ops __attribute__((noderef, address_space(2))) snd_usb_capture_ops = { // MJR_new
...
//#if defined(CONFIG_PROC_FS) && defined(CONFIG_SND_VERBOSE_PROCFS) // MJR -------------------->
				     ...
				     int format, unsigned char *fmt)
{
	...
	case USB_AUDIO_FORMAT_PCM:
		if (sample_width > sample_bytes * 8) {
			mprintk(KERN_INFO "%d:%u:%d : sample bitwidth %d in over sample bytes %d\n",
				   chip->dev->devnum, fp->iface, fp->altsetting,
		...
		case 4:
			...
			break;
		default:
			mprintk(KERN_INFO "%d:%u:%d : unsupported sample bitwidth %d in %d bytes\n",
	...
	case USB_AUDIO_FORMAT_MU_LAW:
		...
		break;
	default:
		mprintk(KERN_INFO "%d:%u:%d : unsupported format type %d\n",
				    ...
				    unsigned char *fmt, int offset)
{
	...
	if (fmt[0] < offset + 1 + 3 * (nr_rates ? nr_rates : 2)) {
		mprintk(KERN_ERR "%d:%u:%d : invalid FORMAT_TYPE desc\n",
				   chip->dev->devnum, fp->iface, fp->altsetting);
		return -1;
	}
	...
	if (nr_rates) {
		...
		if (fp->rate_table == NULL) {
			mprintk(KERN_ERR "cannot malloc\n");
			return -1;
		}
				...
				int format, unsigned char *fmt)
{
	...
	fp->channels = fmt[4];
	if (fp->channels < 1) {
		mprintk(KERN_ERR "%d:%u:%d : invalid channels %d\n",
...
static int parse_audio_endpoints(struct snd_usb_audio *chip, int iface_no)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (!fmt) {
			mprintk(KERN_ERR "%d:%u:%d : AS_GENERAL descriptor not found\n",
				   dev->devnum, iface_no, altno);
			continue;
		}

		if (fmt[0] < 7) {
			mprintk(KERN_ERR "%d:%u:%d : invalid AS_GENERAL desc\n",
				   dev->devnum, iface_no, altno);
			continue;
		}
		...
		if (!fmt) {
			mprintk(KERN_ERR "%d:%u:%d : no FORMAT_TYPE desc\n",
				   dev->devnum, iface_no, altno);
			continue;
		}
		if (fmt[0] < 8) {
		...
		if (!csep || csep[0] < 7 || csep[2] != EP_GENERAL) {
			mprintk(KERN_WARNING "%d:%u:%d : no or invalid"
...
static int snd_usb_create_streams(struct snd_usb_audio *chip, int ctrlif)
{
	...
	if (!(p1 = snd_usb_find_csint_desc(host_iface->extra, host_iface->extralen, NULL, HEADER))) {
		mprintk(KERN_ERR "cannot find HEADER\n");
		return -EINVAL;
	}
	if (! p1[7] || p1[0] < 8 + p1[7]) {
		mprintk(KERN_ERR "invalid HEADER\n");
		return -EINVAL;
	}
	...
	for (i = 0; i < p1[7]; i++) {
		...
		if (!iface) {
			mprintk(KERN_ERR "%d:%u:%d : does not exist\n",
				   dev->devnum, ctrlif, j);
			continue;
		}
		...
		if ((altsd->bInterfaceClass == USB_CLASS_AUDIO ||
			...
			if (snd_usb_create_midi_interface(chip, iface, NULL) < 0) {
				mprintk(KERN_ERR "%d:%u:%d: cannot create sequencer device\n", dev->devnum, ctrlif, j);
				continue;
			}
		...
		if (snd_usb_get_speed(dev) == USB_SPEED_LOW) {
			mprintk(KERN_ERR "low speed audio streaming not supported\n");
			continue;
		}
				     ...
				     const struct snd_usb_audio_quirk *quirk)
{
	...
	if (! fp) {
		mprintk(KERN_ERR "cannot memdup\n");
		return -ENOMEM;
	}
	if (fp->nr_rates > 0) {
				       ...
				       const struct snd_usb_audio_quirk *quirk)
{
	...
	if (err < 0) {
		mprintk(KERN_ERR "cannot setup if %d: error %d\n",
                             ...
                             const struct snd_usb_audio_quirk *quirk)
{
	...
	case 0x260:
		...
		break;
	default:
		mprintk(KERN_ERR "unknown sample rate\n");
		kfree(fp);
		return -ENXIO;
	}
...
{
    // MJR Commented this out because of a type-resolution bug in DriverSlicer
...
{
	struct snd_info_entry *entry = NULL; //MJR
...
static int snd_usb_audio_dev_free(struct snd_device *device)
{
      ...
      // MJR Type resolution issue in DriverSlicer
				...
				struct snd_usb_audio **rchip)
{
	...
	int err, len;
	char component[14];
	static struct __attribute__((noderef, address_space(2))) snd_device_ops ops = { // MJR_new
	...
	*rchip = NULL;
	...
	if (snd_usb_get_speed(dev) != USB_SPEED_LOW &&
		...
		mprintk(KERN_ERR "unknown device speed %d\n", snd_usb_get_speed(dev));
		return -ENXIO;
	}

	card = snd_card_new(index[idx], id[idx], THIS_MODULE, 0);
	if (card == NULL) {
		mprintk(KERN_ERR "cannot create card instance %d\n", idx);
		return -ENOMEM;
	}
	...
	/* retrieve the device string as shortname */
 	if (quirk && quirk->product_name) {
            MJR_strlcpy(card->shortname, quirk->product_name, sizeof(card->shortname));
	} else {
	...
	if (quirk && quirk->vendor_name) {
            len = MJR_strlcpy(card->longname, quirk->vendor_name, sizeof(card->longname));
	} else {
	...
	if (len > 0)
            MJR_strlcat(card->longname, " ", sizeof(card->longname));

	MJR_strlcat(card->longname, card->shortname, sizeof(card->longname));

	len = MJR_strlcat(card->longname, " at ", sizeof(card->longname));

	if (len < sizeof(card->longname))
		usb_make_path(dev, card->longname + len, sizeof(card->longname) - len);

	MJR_strlcat(card->longname,
				 ...
				 const struct usb_device_id *usb_id)
{
	...
	for (i = 0; i < SNDRV_CARDS; i++) {
		if (usb_chip[i] && usb_chip[i]->dev == dev) {
		    if (usb_chip[i]->shutdown) {
		      //  mprintk(KERN_ERR "USB device is in the shutdown state, cannot create a card instance\n");
		      //  goto __error;
		    }
			chip = usb_chip[i];
			break;
	...
	if (! chip) {
		...
		if (!chip) {
			mprintk(KERN_ERR "no available usb audio device\n");
			goto __error;
		}
	}

        mprintk ("%s Location 6\n", __FUNCTION__); // MJR
	...
	if (err > 0) {
        ...
        mprintk ("%s Location 8\n", __FUNCTION__); // MJR
      
	/* we are allowed to call snd_card_register() many times */
        { // MJR modified this section
            int error = snd_card_register(chip->card);
            
            if (error < 0) { // MJR modified
                mprintk ("%s failure: %d\n", __FUNCTION__, error);
		goto __error;
            }
...
struct usb_device *Nonstub_get_usb_device (struct usb_interface *intf,
        int marshaling,
        int gen_kern,
        struct usb_device *after) {
    struct usb_device *dev = container_of (intf->dev.parent, struct usb_device, dev);
    if (gen_kern) {
        if (marshaling) {
            if (!after) {
            } else {
...
// MJR interface_to_usbdev(intf):  container_of(intf->dev.parent, struct
// usb_device, dev)
//
//  MJR This does not work on the intf parameter:
//                          __attribute__((conttype(sizeof(struct usb_device))))
//                          __attribute__((contfield("dev")))
//  MJR Need to do this for intf->dev.parent
//
...
static int usb_audio_probe(struct usb_interface *
                           __attribute__((extraptr(sizeof (struct usb_device *), "Nonstub_get_usb_device")))intf, // MJR
                           const struct usb_device_id *id)
{
    void *chip;
...
static void usb_audio_disconnect(struct usb_interface *
                                 __attribute__((noderef, address_space(2)))
                                 /*__attribute__((extraptr(sizeof (struct usb_device *), "Nonstub_get_usb_device")))*/ // MJR
                                 intf)
{
...
static int __init snd_usb_audio_init(void)
{
	...
	if (nrpacks < MIN_PACKS_URB || nrpacks > MAX_PACKS) {
		mprintk(KERN_WARNING "invalid nrpacks value.\n");
		return -EINVAL;
	}
