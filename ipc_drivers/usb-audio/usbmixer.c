 ...
 */
static const struct __attribute__((noderef, address_space(2))) rc_config { // MJR_new
	u32 usb_id;
	u8  offset;
	u8  length;
	u8  packet_length;
...
struct mixer_build {
    ...
    unsigned char *__attribute__((expfld(buflen))) buffer;
	unsigned int buflen;
...
struct usb_mixer_elem_info {
	struct usb_mixer_interface *mixer;
    struct usb_mixer_elem_info *__attribute__((recursive)) next_id_elem; /* list of controls with same id */ // MJR
...
static int check_mapped_name(struct mixer_build *state, int unitid, int control,
                             char * __attribute__((exp(Nonstub_return_buflen("buflen")))) buf, // MJR
                             int buflen)
{
	...
	for (p = state->map; p->id; p++) {
		if (p->id == unitid && p->name &&
			...
			buflen--;
			return MJR_strlcpy(buf, p->name, buflen);
		}
	}
	return 0;
}
                                      ...
                                      int buflen)
{
	...
	for (p = state->selector_map; p->id; p++) {
		if (p->id == unitid && index < p->count)
			return MJR_strlcpy(buf, p->names[index], buflen);
	}
	return 0;
}
 ...
 */
static int snd_usb_copy_string_desc(struct mixer_build *state, int index,
                                    char *__attribute__((exp(Nonstub_return_buflen("buflen")))) buf, // MJR
                                    int maxlen)
{
...
static struct __attribute__((noderef, address_space(2))) iterm_name_combo { // MJR_new
	int type;
...
static struct usb_feature_control_info (__attribute__((noderef, address_space(2))) audio_feature_info)[] = { // MJR_new
...
static struct snd_kcontrol_new __attribute__((noderef, address_space(2))) usb_feature_unit_ctl = { // MJR
			      ...
			      struct usb_audio_term *iterm, int unitid)
{
	...
	if (! cval) {
		snd_mprintk(KERN_ERR "cannot malloc kcontrol\n");
		return;
	}
	...
	case USB_FEATURE_VOLUME:
		...
		if (! len) {
			...
			if (! len)
                            ...
                            // "Feature %d", unitid); // MJR LLVM issue
                            len = sprintf(kctl->id.name, "Feature %d", unitid); // MJR LLVM issue
		}
		...
		if (! mapped_name && ! (state->oterm.type >> 16)) {
			if ((state->oterm.type & 0xff00) == 0x0100) {
				len = MJR_strlcat(kctl->id.name, " Capture", sizeof(kctl->id.name));
			} else {
				len = MJR_strlcat(kctl->id.name + len, " Playback", sizeof(kctl->id.name));
			}
		}
		MJR_strlcat(kctl->id.name + len, control == USB_FEATURE_MUTE ? " Switch" : " Volume",
			sizeof(kctl->id.name));
	...
	default:
		if (! len)
			MJR_strlcpy(kctl->id.name, audio_feature_info[control-1].name,
				sizeof(kctl->id.name));
		break;
	}
	...
	case USB_ID(0x0672, 0x1041):
		if (!strcmp(kctl->id.name, "PCM Playback Volume") &&
		    cval->min == -15616) {
			snd_mprintk(KERN_INFO "using volume control quirk for the UDA1321/N101 chip\n");
			cval->max = -256;
		}
	}
...
static int parse_audio_feature_unit(struct mixer_build *state, int unitid, unsigned char *ftr)
{
	...
	if (ftr[0] < 7 || ! (csize = ftr[5]) || ftr[0] < 7 + csize) {
		snd_mprintk(KERN_ERR "usbaudio: unit %u: invalid FEATURE_UNIT descriptor\n", unitid);
		return -EINVAL;
	}
				 ...
				 struct usb_audio_term *iterm)
{
	...
	MJR_strlcat(kctl->id.name + len, " Volume", sizeof(kctl->id.name));
...
static int parse_audio_mixer_unit(struct mixer_build *state, int unitid, unsigned char *desc)
{
	...
	if (desc[0] < 11 || ! (input_pins = desc[4]) || ! (num_outs = desc[5 + input_pins])) {
		snd_mprintk(KERN_ERR "invalid MIXER UNIT descriptor %d\n", unitid);
		return -EINVAL;
	}
...
static struct snd_kcontrol_new __attribute__((noderef, address_space(2))) mixer_procunit_ctl = { // MJR_new
	.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
...
static struct procunit_value_info (__attribute__((noderef, address_space(2))) updown_proc_info)[] = { // MJR_new
	{ USB_PROC_UPDOWN_SWITCH, "Switch", USB_MIXER_BOOLEAN },
	{ USB_PROC_UPDOWN_MODE_SEL, "Mode Select", USB_MIXER_U8, 1 },
	{ 0 }
};
static struct procunit_value_info (__attribute__((noderef, address_space(2))) prologic_proc_info)[] = { // MJR_new
	{ USB_PROC_PROLOGIC_SWITCH, "Switch", USB_MIXER_BOOLEAN },
	{ USB_PROC_PROLOGIC_MODE_SEL, "Mode Select", USB_MIXER_U8, 1 },
	{ 0 }
};
static struct procunit_value_info (__attribute__((noderef, address_space(2))) threed_enh_proc_info)[] = { // MJR_new
	{ USB_PROC_3DENH_SWITCH, "Switch", USB_MIXER_BOOLEAN },
	{ USB_PROC_3DENH_SPACE, "Spaciousness", USB_MIXER_U8 },
	{ 0 }
};
static struct procunit_value_info (__attribute__((noderef, address_space(2))) reverb_proc_info)[] = { // MJR_new
...
static struct procunit_value_info (__attribute__((noderef, address_space(2))) chorus_proc_info)[] = { // MJR_new
...
static struct procunit_value_info (__attribute__((noderef, address_space(2))) dcr_proc_info)[] = { // MJR_new
...
static struct procunit_info (__attribute__((noderef, address_space(2))) procunits)[] = { // MJR_new
...
static int build_audio_procunit(struct mixer_build *state, int unitid, unsigned char *dsc, struct procunit_info *list, char *name)
{
	...
	static struct procunit_value_info (__attribute__((noderef, address_space(2))) default_value_info)[] = { // MJR_new
		{ 0x01, "Switch", USB_MIXER_BOOLEAN },
		{ 0 }
	};
	static struct procunit_info __attribute__((noderef, address_space(2))) default_info = { // MJR_new
		0, NULL, default_value_info
	};

	if (dsc[0] < 13 || dsc[0] < 13 + num_ins || dsc[0] < num_ins + dsc[11 + num_ins]) {
		snd_mprintk(KERN_ERR "invalid %s descriptor (id %d)\n", name, unitid);
		return -EINVAL;
	}
	...
	for (valinfo = info->values; valinfo->control; valinfo++) {
		...
		if (check_mapped_name(state, unitid, cval->control, kctl->id.name, sizeof(kctl->id.name)))
			...
			MJR_strlcpy(kctl->id.name, info->name, sizeof(kctl->id.name));
		else {
			...
			if (nameid)
				len = snd_usb_copy_string_desc(state, nameid, kctl->id.name, sizeof(kctl->id.name));
			if (! len)
				MJR_strlcpy(kctl->id.name, name, sizeof(kctl->id.name));
		}
		MJR_strlcat(kctl->id.name, " ", sizeof(kctl->id.name));
		MJR_strlcat(kctl->id.name, valinfo->suffix, sizeof(kctl->id.name));
...
static struct snd_kcontrol_new __attribute__((noderef, address_space(2))) mixer_selectunit_ctl = { // MJR_new
	.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
	.name = "", /* will be filled later */
...
static int parse_audio_selector_unit(struct mixer_build *state, int unitid, unsigned char *desc)
{
	...
	if (! num_ins || desc[0] < 5 + num_ins) {
		snd_mprintk(KERN_ERR "invalid SELECTOR UNIT descriptor %d\n", unitid);
		return -EINVAL;
	}
	...
	if (! namelist) {
		snd_mprintk(KERN_ERR "cannot malloc\n");
		kfree(cval);
		return -ENOMEM;
	}
#define MAX_ITEM_NAME_LEN	64
	...
	if (len)
		...
		if (! len)
			MJR_strlcpy(kctl->id.name, "USB", sizeof(kctl->id.name));

		if ((state->oterm.type & 0xff00) == 0x0100)
                    MJR_strlcat(kctl->id.name, " Capture Source", sizeof(kctl->id.name));
		else
                    MJR_strlcat(kctl->id.name, " Playback Source", sizeof(kctl->id.name));
	}
...
static int parse_audio_unit(struct mixer_build *state, int unitid)
{
	...
	if (!p1) {
		snd_mprintk(KERN_ERR "usbaudio: unit %d not found!\n", unitid);
		return -EINVAL;
	}
	...
	default:
		snd_mprintk(KERN_ERR "usbaudio: unit %u: unexpected type 0x%02x\n", unitid, p1[2]);
		return -EINVAL;
	}
}
...
static int snd_usb_soundblaster_remote_init(struct usb_mixer_interface *mixer)
{
	...
	//snprintf(hwdep->name, sizeof(hwdep->name),
        // "%s remote control", mixer->chip->card->shortname); // MJR LLVM issue
        sprintf(hwdep->name, "%s remote control", mixer->chip->card->shortname); // MJR LLVM issue
...
static struct snd_kcontrol_new (__attribute__((noderef, address_space(2))) snd_audigy2nx_controls)[] = { // MJR_new
	{
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
...
int snd_usb_create_mixer(struct snd_usb_audio *chip, int ctrlif,
			 int ignore_error)
{
    static struct snd_device_ops __attribute__((noderef, address_space(2))) dev_ops = { // MJR_new
