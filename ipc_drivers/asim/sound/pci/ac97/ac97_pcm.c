...
/* static MJR */ unsigned char get_slot_reg(struct ac97_pcm *pcm, unsigned short cidx,
...
/* static MJR */ int set_spdif_rate(struct snd_ac97 *ac97, unsigned short rate)
{
...
// EXPORT_SYMBOL(snd_ac97_set_rate); // MJR

/* static MJR */ unsigned short get_pslots(struct snd_ac97 *ac97, unsigned char *rate_table, unsigned short *spdif_slots)
{
...
/* static MJR */ unsigned short get_cslots(struct snd_ac97 *ac97)
{
	unsigned short slots;
...
/* static MJR */ unsigned int get_rates(struct ac97_pcm *pcm, unsigned int cidx, unsigned short slots, int dbl)
{
	int i, idx;
...
// EXPORT_SYMBOL(snd_ac97_pcm_assign); // MJR
		      ...
		      enum ac97_pcm_cfg cfg, unsigned short slots)
{
	...
	for (i = 3; i < 12; i++) {
		...
		if (!ok_flag) {
			spin_unlock_irq(&pcm->bus->bus_lock);
			snd_mprintk(KERN_ERR "cannot find configuration for AC97 slot %i\n", i); // MJR
			err = -EAGAIN;
			goto error;
		}
	}
	...
	for (i = 3; i < 12; i++) {
		...
		for (cidx = 0; cidx < 4; cidx++) {
			if (pcm->r[r].rslots[cidx] & (1 << i)) {
				...
				if (reg == 0xff) {
					snd_mprintk(KERN_ERR "invalid AC97 slot %i?\n", i); // MJR
					continue;
				}
				...
				if (err < 0)
					snd_mprintk(KERN_ERR "error in snd_ac97_set_rate: cidx=%d, reg=0x%x, rate=%d, err=%d\n", cidx, reg, rate, err); // MJR
				else
...
// EXPORT_SYMBOL(snd_ac97_pcm_open); // MJR
...
// EXPORT_SYMBOL(snd_ac97_pcm_close); // MJR

/* static MJR */ int double_rate_hw_constraint_rate(struct snd_pcm_hw_params *params,
...
/* static MJR */ int double_rate_hw_constraint_channels(struct snd_pcm_hw_params *params,
					      struct snd_pcm_hw_rule *rule)
{
	struct snd_interval *rate = hw_param_interval(params, SNDRV_PCM_HW_PARAM_RATE);
	if (rate->min > 48000) {
...
int snd_ac97_pcm_double_rate_rules(struct snd_pcm_runtime *runtime)
{
	...
	if (err < 0)
		return err;
	err = snd_pcm_hw_rule_add_MJR1(runtime, 0, SNDRV_PCM_HW_PARAM_CHANNELS,
				  double_rate_hw_constraint_channels, NULL,
				  SNDRV_PCM_HW_PARAM_RATE, -1);
	return err;
}

// EXPORT_SYMBOL(snd_ac97_pcm_double_rate_rules); // MJR
