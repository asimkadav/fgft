...
/* static MJR */ void snd_midi_input_event(struct snd_rawmidi_substream *substream)
{
...
/* static MJR */ int dump_midi(struct snd_rawmidi_substream *substream, const char *buf, int count)
{
	struct snd_rawmidi_runtime *runtime;
	int tmp;
...
/* static MJR */ int event_process_midi(struct snd_seq_event *ev, int direct,
...
/* static MJR */ int snd_seq_midisynth_new(struct seq_midisynth *msynth,
...
/* static MJR */ int midisynth_subscribe(void *private_data, struct snd_seq_port_subscribe *info)
{
	int err;
	struct seq_midisynth *msynth = private_data;
	struct snd_rawmidi_runtime *runtime;
...
/* static MJR */ int midisynth_unsubscribe(void *private_data, struct snd_seq_port_subscribe *info)
{
	int err;
	struct seq_midisynth *msynth = private_data;
...
/* static MJR */ int midisynth_use(void *private_data, struct snd_seq_port_subscribe *info)
{
	int err;
	struct seq_midisynth *msynth = private_data;
	struct snd_rawmidi_params params;
...
/* static MJR */ int midisynth_unuse(void *private_data, struct snd_seq_port_subscribe *info)
{
	struct seq_midisynth *msynth = private_data;
...
/* static MJR */ void snd_seq_midisynth_delete(struct seq_midisynth *msynth)
{
	if (msynth == NULL)
		return;
				...
				if (ports > 1)
					...
					snprintf(port->name, sizeof(port->name), "%s", info->name);
                            */
                            ...
                            if (ports > 1)
                                sprintf(temp, "%s-%d", info->name, p);
                            else
                                sprintf(temp, "%s", info->name);

                            memcpy (port->name, temp, sizeof(port->name));
...
/* static MJR */ int /* __init MJR */  alsa_seq_midi_init(void)
{
...
/* static MJR */ void __exit alsa_seq_midi_exit(void)
{
	snd_seq_device_unregister_driver(SNDRV_SEQ_DEV_ID_MIDISYNTH);
}

// module_init(alsa_seq_midi_init) // MJR
// module_exit(alsa_seq_midi_exit) // MJR
