 ...
 * create a port
 */
static struct snd_seq_dummy_port /* __init MJR */  *
...
register_client(void)
{
	...
	if (ports < 1) {
		snd_mprintk(KERN_ERR "invalid number of ports %d\n", ports); // MJR
		return -EINVAL;
	}
...
/* static MJR */ int /* __init MJR */  alsa_seq_dummy_init(void)
{
	int err;
...
/* static MJR */ void __exit alsa_seq_dummy_exit(void)
{
	delete_client();
}

// module_init(alsa_seq_dummy_init) // MJR
// module_exit(alsa_seq_dummy_exit) // MJR
