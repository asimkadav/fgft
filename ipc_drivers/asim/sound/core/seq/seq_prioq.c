...
/* return 1 if a >= b; 0 */
/* static MJR */ inline int compare_timestamp(struct snd_seq_event *a,
 ...
 *        positive if a > b;
 */
/* static MJR */ inline int compare_timestamp_rel(struct snd_seq_event *a,
					struct snd_seq_event *b)
{
	if ((a->flags & SNDRV_SEQ_TIME_STAMP_MASK) == SNDRV_SEQ_TIME_STAMP_TICK) {
		/* compare ticks */
			  ...
			  struct snd_seq_event_cell * cell)
{
	...
	while (cur != NULL) {
		...
		if (! --count) {
			spin_unlock_irqrestore(&f->lock, flags);
			snd_mprintk(KERN_ERR "cannot find a pointer.. infinite loop?\n"); // MJR
			return -EINVAL;
		}
	}
...
/* static MJR */ inline int prioq_match(struct snd_seq_event_cell *cell,
...
#if 0
			mprintk("type = %i, source = %i, dest = %i, client = %i\n", // MJR
				cell->event.type,
...
/* static MJR */ int prioq_remove_match(struct snd_seq_remove_events *info,
