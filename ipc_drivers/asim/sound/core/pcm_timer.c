...
/* static MJR */ unsigned long gcd(unsigned long a, unsigned long b)
{
	unsigned long r;
	if (a < b) {
		r = a;
...
void snd_pcm_timer_resolution_change(struct snd_pcm_substream *substream)
{
	...
	if (rate == 0) {
		snd_mprintk(KERN_ERR "pcm timer resolution out of range (rate = %u, period_size = %lu)\n", runtime->rate, runtime->period_size); // MJR
...
/* static MJR */ unsigned long snd_pcm_timer_resolution(struct snd_timer * timer)
{
...
/* static MJR */ int snd_pcm_timer_start(struct snd_timer * timer)
{
	unsigned long flags;
	struct snd_pcm_substream *substream;
...
/* static MJR */ int snd_pcm_timer_stop(struct snd_timer * timer)
{
	unsigned long flags;
	struct snd_pcm_substream *substream;
...
/* static MJR */ void snd_pcm_timer_free(struct snd_timer *timer)
{
