...
// EXPORT_SYMBOL(snd_opl3_regmap); // MJR

/*
 * prototypes
 */
/* static MJR */ int snd_opl3_play_note(struct snd_opl3 * opl3, struct snd_dm_fm_note * note);
/* static MJR */ int snd_opl3_set_voice(struct snd_opl3 * opl3, struct snd_dm_fm_voice * voice);
/* static MJR */ int snd_opl3_set_params(struct snd_opl3 * opl3, struct snd_dm_fm_params * params);
/* static MJR */ int snd_opl3_set_mode(struct snd_opl3 * opl3, int mode);
/* static MJR */ int snd_opl3_set_connection(struct snd_opl3 * opl3, int connection);
		   ...
		   unsigned int cmd, unsigned long arg)
{
	...
	default:
		snd_mprintk("unknown IOCTL: 0x%x\n", cmd); // MJR
#endif
	}
	return -ENOTTY;
}
			...
			const unsigned char *data)
{
	...
	if (name)
		strncpy(patch->name, name, sizeof(patch->name)); // MJR

	return 0;
}
// EXPORT_SYMBOL(snd_opl3_load_patch); // MJR
...
// EXPORT_SYMBOL(snd_opl3_find_patch); // MJR
...
// EXPORT_SYMBOL(snd_opl3_reset); // MJR
