			      ...
			      struct urb *urb)
{
	...
	for (i = 0; i < urb->number_of_packets; i++) {
		...
		if (urb->iso_frame_desc[i].status) {
			printk(KERN_ERR "frame %d active: %d\n", i, urb->iso_frame_desc[i].status);
			// continue;
		}
...
/*
 */
static struct snd_urb_ops (__attribute__((noderef, address_space(2))) audio_urb_ops)[2] = { // new
	{
...
static struct snd_urb_ops (__attribute__((noderef, address_space(2))) audio_urb_ops_high_speed)[2] = { // new
	{
		.prepare =	prepare_nodata_playback_urb,
		.retire =	retire_playback_urb,
...
static void snd_complete_urb(struct urb *urb)
{
	...
	if ((subs->running && subs->ops.retire(subs, substream->runtime, urb)) ||
		...
		if (err < 0) {
			printk(KERN_ERR "cannot submit urb (err = %d)\n", err);
...
static void snd_complete_sync_urb(struct urb *urb)
{
	...
	if ((subs->running && subs->ops.retire_sync(subs, substream->runtime, urb)) ||
		...
		if (err < 0) {
			printk(KERN_ERR "cannot submit sync urb (err = %d)\n", err);
			snd_pcm_stop(substream, SNDRV_PCM_STATE_XRUN);
		}
	}
}
				...
				struct audioformat *fmt, int rate)
{
	...
	if (fmt->attributes & EP_CS_ATTR_SAMPLE_RATE) {
		...
		if (crate != rate) {
			printk(KERN_WARNING "current rate %d is different from the runtime rate %d\n", crate, rate);
			// runtime->rate = crate;
		}
	}
	return 0;
...
static int set_format(struct snd_usb_substream *subs, struct audioformat *fmt)
{
	...
	if (subs->interface != fmt->iface || subs->format != fmt->altset_idx) {
		if (usb_set_interface(dev, fmt->iface, fmt->altsetting) < 0) {
			printk(KERN_ERR "%d:%d:%d: usb_set_interface failed\n",
				   dev->devnum, fmt->iface, fmt->altsetting);
			return -EIO;
		}
		printk(KERN_INFO "setting usb interface %d:%d\n", fmt->iface, fmt->altsetting);
	...
	if (((is_playback && attr == EP_ATTR_ASYNC) ||
		...
		if ((get_endpoint(alts, 1)->bmAttributes & USB_ENDPOINT_XFERTYPE_MASK) != 0x01 ||
			...
			printk(KERN_ERR "%d:%d:%d : invalid synch pipe\n",
				   dev->devnum, fmt->iface, fmt->altsetting);
			return -EINVAL;
		}
			     ...
			     struct snd_pcm_hw_params *hw_params)
{
	...
	if (!fmt) {
		printk(KERN_DEBUG "cannot set format: format = 0x%x, rate = %d, channels = %d\n",
...
static struct snd_pcm_hardware __attribute__((noderef, address_space(2))) snd_usb_hardware = // new
{
...
static struct snd_pcm_ops __attribute__((noderef, address_space(2))) snd_usb_playback_ops = { // new
...
static struct snd_pcm_ops __attribute__((noderef, address_space(2))) snd_usb_capture_ops = { // new
				     ...
				     int format, unsigned char *fmt)
{
	...
	case 0: /* some devices don't define this correctly... */
		printk(KERN_INFO "%d:%u:%d : format type 0 is detected, processed as PCM\n",
	...
	case USB_AUDIO_FORMAT_PCM:
		...
		case 4:
			...
			break;
		default:
			printk(KERN_INFO "%d:%u:%d : unsupported sample bitwidth %d in %d bytes\n",
	...
	case USB_AUDIO_FORMAT_MU_LAW:
		...
		break;
	default:
		printk(KERN_INFO "%d:%u:%d : unsupported format type %d\n",
				 ...
				 int format, unsigned char *fmt)
{
	...
	case USB_AUDIO_FORMAT_MPEG:
		...
		break;
	default:
		printk(KERN_INFO "%d:%u:%d : unknown format tag 0x%x is detected.  processed as MPEG.\n",
			   chip->dev->devnum, fp->iface, fp->altsetting, format);
		fp->format = SNDRV_PCM_FORMAT_MPEG;
		break;
	}
	...
	printk(KERN_INFO "found format II with max.bitrate = %d, frame size=%d\n", brate, framesize);
			      ...
			      int format, unsigned char *fmt, int stream)
{
	...
	case USB_FORMAT_TYPE_II:
		...
		break;
	default:
		printk(KERN_INFO "%d:%u:%d : format type %d is not supported yet\n",
...
static int parse_audio_endpoints(struct snd_usb_audio *chip, int iface_no)
{
	...
	for (i = 0; i < num; i++) {
		...
		printk(KERN_INFO "%d:%u:%d: add audio endpoint 0x%x\n", dev->devnum, iface_no, altno, fp->endpoint);
...
static int snd_usb_create_streams(struct snd_usb_audio *chip, int ctrlif)
{
	...
	for (i = 0; i < p1[7]; i++) {
		...
		if (usb_interface_claimed(iface)) {
			printk(KERN_INFO "%d:%d:%d: skipping, already claimed\n", dev->devnum, ctrlif, j);
			continue;
		}
		...
		if ((altsd->bInterfaceClass != USB_CLASS_AUDIO &&
		     altsd->bInterfaceClass != USB_CLASS_VENDOR_SPEC) ||
		    altsd->bInterfaceSubClass != USB_SUBCLASS_AUDIO_STREAMING) {
			printk(KERN_ERR "%d:%u:%d: skipping non-supported interface %d\n", dev->devnum, ctrlif, j, altsd->bInterfaceClass);
...
static int snd_usb_extigy_boot_quirk(struct usb_device *dev, struct usb_interface *intf)
{
	...
	if (le16_to_cpu(get_cfg_desc(config)->wTotalLength) == EXTIGY_FIRMWARE_SIZE_OLD ||
		...
		if (err < 0) printk("error sending boot message: %d\n", err);
		err = usb_get_descriptor(dev, USB_DT_DEVICE, 0,
				&dev->descriptor, sizeof(dev->descriptor));
		config = dev->actconfig;
		if (err < 0) printk("error usb_get_descriptor: %d\n", err);
		err = usb_reset_configuration(dev);
		if (err < 0) printk("error usb_reset_configuration: %d\n", err);
				...
				struct snd_usb_audio **rchip)
{
	...
	int err, len;
	char component[14];
	static struct __attribute__((noderef, address_space(2))) snd_device_ops ops = { // new
