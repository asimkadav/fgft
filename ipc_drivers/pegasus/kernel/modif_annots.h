...
#define MODIFIES(var)                                                   \
    do {                                                                \
        (void) var;                                                     \
        MICRODRIVERS__DUMMY((unsigned long) var);                       \
    } while (0)
...
struct usb_device *MODIFANNOT(usb_get_dev)(struct usb_device *dev) {
    return NULL;
}

struct usb_device *MODIFANNOT(usb_put_dev)(struct usb_device *dev) {
    return NULL;
}
